const vlogrounds = [
    {
        id: '221c5dc5-45db-4e76-8bbe-0a6c92a9325a',
        createdDate: new Date(),
        updatedDate: new Date(),
        tontineId: 'phuongvlog',
        dateStart: new Date(2019, 4, 1),
        dateEnd: new Date(2019, 8, 30),
        schedule: [
            '5e4eff8b6b2693d84ae7db72',
            '7cee9e30-62d1-46dc-97ea-bf2e6b8f7441',
            'e8842900-0685-4894-8df2-66a4821c4f53',
            'cbf08139-3dba-4120-8406-93243f2353f6',
            '29c0e911-9c18-497a-b9ef-b10fb170f765'
        ]
    },
    {
        id: '645373f9-d0e3-4f8b-9c50-29defe69b74b',
        createdDate: new Date(),
        updatedDate: new Date(),
        tontineId: 'phuongvlog',
        dateStart: new Date(2019, 9, 1),
        dateEnd: new Date(2020, 8, 30),
        schedule: [
            {
                investorId: '29c0e911-9c18-497a-b9ef-b10fb170f765',
                dateStart: new Date(2019, 9, 1),
                dateEnd: new Date(2019, 9, 31)
            },
            {
                investorId: 'e8842900-0685-4894-8df2-66a4821c4f53',
                dateStart: new Date(2019, 10, 1),
                dateEnd: new Date(2019, 10, 30)
            },
            {
                investorId: '5e4f00915cac3cd96235a2c1',
                dateStart: new Date(2019, 11, 1),
                dateEnd: new Date(2019, 11, 31)
            },
            {
                investorId: '7cee9e30-62d1-46dc-97ea-bf2e6b8f7441',
                dateStart: new Date(2020, 0, 1),
                dateEnd: new Date(2020, 0, 31)
            },
            {
                investorId: 'cbf08139-3dba-4120-8406-93243f2353f6',
                dateStart: new Date(2020, 1, 1),
                dateEnd: new Date(2020, 1, 28)
            },
            {
                investorId: '5e4eff8b6b2693d84ae7db72',
                dateStart: new Date(2020, 2, 1),
                dateEnd: new Date(2020, 2, 31)
            },
            {
                investorId: '5e4f00d51f6741d97dad58ac',
                dateStart: new Date(2020, 3, 1),
                dateEnd: new Date(2020, 3, 30)
            },
            {
                investorId: '5e4effd52aaa50d92e632033',
                dateStart: new Date(2020, 4, 1),
                dateEnd: new Date(2020, 4, 31)
            },
            {
                investorId: '5e4f00915cac3cd96235a2c1',
                dateStart: new Date(2020, 5, 1),
                dateEnd: new Date(2020, 5, 30)
            },
            {
                investorId: '5e4f0010f398dbd940e8661b',
                dateStart: new Date(2020, 6, 1),
                dateEnd: new Date(2020, 6, 31)
            },
            {
                investorId: 'e8842900-0685-4894-8df2-66a4821c4f53',
                dateStart: new Date(2020, 7, 1),
                dateEnd: new Date(2020, 7, 31)
            },
            {
                investorId: '5e4f00915cac3cd96235a2c1',
                dateStart: new Date(2020, 8, 1),
                dateEnd: new Date(2020, 8, 30)
            }
        ]
    }
];

module.exports = vlogrounds;