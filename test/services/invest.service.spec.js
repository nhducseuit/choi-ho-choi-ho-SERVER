...
describe('InvestService', function () {
    ...
    describe('invest', () => {
        ...
        it('should correctly update investment to database when profile investment has not yet initiated', async () => {
            investProfile.investments = null;
            investProfile = vloggersInvestProfiles.find(profile => profile.investorId === validInvestorId);
            mongoService.findById = sinon.fake.returns(validRound);
            mockCollection.findOne = sinon.fake.returns(investProfile);
            mockCollection.updateOne = sinon.fake.returns({
            ...
            expect(await investService.invest(validInvestorId, roundId, investDate)).to.be.true;
            expect(investProfile.status).to.be.equal(InvestStatus.ACTIVE);
            expect(investProfile.investments).to.be.eql([Invest.invest({
                date: new Date(investDate),
                amount: investProfile.annualDepositeAmount,
                type: -1
            })]);
        });
        it('should throw exception if update result is not OK', async () => {
            investProfile = vloggersInvestProfiles.find(profile => profile.investorId === validInvestorId);
            mongoService.findById = sinon.fake.returns(validRound);
            mockCollection.findOne = sinon.fake.returns(investProfile);
            mockCollection.updateOne = sinon.fake.returns({
            ...
            await expect(investService.invest(validInvestorId, roundId, investDate)).to.be.rejectedWith(ServerException, 'Failed to persist investment updates to database');
        });
        it('should throw exception if update throws error', async () => {
            investProfile = vloggersInvestProfiles.find(profile => profile.investorId === validInvestorId);
            mongoService.findById = sinon.fake.returns(validRound);
            mockCollection.findOne = sinon.fake.returns(investProfile);
            mockCollection.updateOne = sinon.fake.throws(new Error());
            await expect(investService.invest(validInvestorId, roundId, investDate)).to.be.rejectedWith(ServerException, 'Failed to persist investment updates to database');
        });
    ...
    describe('getInvestment', () => {
        ...
        it('should throw exception when round is not found in database', async () => {
            mongoService.findById = sinon.fake.returns(null);
            await expect(investService.getInvestment(investorId, roundId, investDate)).to.be.rejectedWith(ServerException, 'Round not found!');
        });
        it('should throw exception when invest profile is not found', async () => {
            mongoService.findById = sinon.fake.returns({});
            mockCollection.findOne = sinon.fake.returns(null);
            await expect(investService.getInvestment(investorId, roundId, investDate)).to.be.rejectedWith(ServerException, 'Invest profile not found!');
        });
        it('should correctly return investment when provided invest information is valid', async () => {
            investProfile = vloggersInvestProfiles.find(profile => profile.investorId === validInvestorId);
            mongoService.findById = sinon.fake.returns(validRound);
            mockCollection.findOne = sinon.fake.returns(investProfile);
            expect(await investService.getInvestment(validInvestorId, roundId, investDate)).to.be.eql(Invest.invest({
                date: new Date(investDate),
                amount: investProfile.annualDepositeAmount,
                type: -1
            }));
